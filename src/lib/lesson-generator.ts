...
function buildMathLesson(concept: string, child: Child, minutes: number) {
  ...
  return {
    ...
    segments: [
      seg('text', 'Discover', 'Let\u2019s explore ' + tl + '. We can use pictures and models to make numbers come alive. Think about what you already know \u2014 what patterns do you notice?', 'Read carefully and think about what you already know about this topic.'),
      seg('interactive', 'Try It', 'Now it\u2019s your turn to work with ' + tl + '. Remember: there\u2019s no rush. Take your time and think step by step.', 'Practice ' + tl + ' with these examples. Try drawing a picture to help you visualize.'),
...
function buildLALesson(concept: string, child: Child, minutes: number) {
  ...
  return {
    ...
    segments: [
      seg('text', 'Read & Listen', 'Great readers and writers have always used ' + tl + ' as a building block. The best way to learn is through living examples \u2014 real stories, real ideas, not just rules.', 'Read this passage carefully. What stands out to you?'),
...
function buildScienceLesson(concept: string, child: Child, minutes: number) {
  ...
  return {
    ...
    segments: [
      seg('text', 'Observe', 'Every great discovery starts with noticing something interesting. ' + t + ' is all around us \u2014 something you can see, touch, or wonder about. Let\u2019s look closer.', 'Read and think: what do you already know about this topic? What questions do you have?'),
      seg('interactive', 'Investigate', 'Time to think like a scientist! Let\u2019s explore ' + tl + ' through a hands-on activity. The best learning happens when you struggle a little before finding the answer.', 'Design a simple experiment or observation about ' + tl + '. What would you test?'),
...
function buildSSLesson(concept: string, child: Child, minutes: number) {
  ...
  return {
    ...
    segments: [
      seg('text', 'Explore', t + ' is part of how human societies work. Let\u2019s start with a big question: Why does this matter? How does it affect people\u2019s lives?', 'Read and think about how this topic connects to your own community or family.'),
      seg('interactive', 'Investigate', 'Let\u2019s dig deeper into ' + tl + '. A great way to understand big ideas is to break them into smaller parts and find patterns.', 'What are the most important parts of ' + tl + '? Try to list 3 key ideas.'),

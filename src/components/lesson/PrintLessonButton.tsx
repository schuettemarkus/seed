...
export function PrintLessonButton({ lesson }: Props) {
  function handlePrint() {
    ...
    printWindow.document.write(`
      ...
      <body>
        <h1>${lesson.title}</h1>
        ${lesson.hook ? `<p class="hook">${lesson.hook}</p>` : ''}

        ${segments.map((seg) => `
          <div class="segment">
            <div class="segment-type">${seg.type.replace('_', ' ')}</div>
            <h2>${seg.title}</h2>
            <p>${seg.content}</p>
            ${seg.instructions ? `<div class="instructions">${seg.instructions}</div>` : ''}
          </div>
        `).join('')}
        ...
        <div class="meta">
          <p>${lesson.subject} · ${lesson.concept_node?.replace(/-/g, ' ')} · ${lesson.estimated_minutes ?? '?'} min</p>
